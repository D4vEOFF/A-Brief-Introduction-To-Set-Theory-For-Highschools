...
\begin{proof}
    ...
    Ukážeme pouze platnost \ref{item:de_morgan_1}, avšak důkaz \ref{item:de_morgan_2} je zcela analogický.
    \textit{($\implies$)}. Budiž dán libovolný prvek $x\in A \setminus \left(\bigcup_{i=1}^{n}{X_i}\right)$. Ukážeme, že $x\in\bigcap_{i=1}^{n}{(A \setminus X_i)}$. Z definice rozdílu množin (viz \ref{def:prunik_rozdil}) musí podle předpokladu platit
    \begin{equation*}
    ...
    \textit{($\impliedby$)}. Podobně ukážeme opačnou implikaci, tj. pokud $x\in\bigcap_{i=1}^{n}{(A \setminus X_i)}$, pak $x\in A\setminus\left(\bigcup_{i=1}^{n}{X_i}\right)$. Nechť je tedy dáno $x\bigcap_{i=1}^{n}{(A \setminus X_i)}$. Z předchozího již víme, že
    \begin{align*}
