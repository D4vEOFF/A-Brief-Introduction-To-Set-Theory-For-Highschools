...
\begin{equation*}
    \forall x \big(P(x) \land Z(x) \land M(x)\big),
\end{equation*}
...
\begin{equation}\label{eq:exist_kvatifikator_priklad}
    \exists x \big(P(x) \land Z(x) \land M(x)\big).
\end{equation}
...
\begin{equation}\label{eq:exist_kvatifikator_priklad_implikace}
    \exists x \big(P(x) \implies Z(x) \land M(x)\big),
\end{equation}
...
Vzpomeneme-li si na definici výrokových formulí (viz \ref{def:vyrokova_a_atomicka_formule}), tak zde jsme formule tvořili opakovanou aplikací jistých pravidel, přičemž "nejtriviálnější" výrokovou formulí (tj. atomickou formulí) pro nás byly \textbf{výrokové proměnné}. Těm jsme přiřazovali pravdivostní hodnotu 0 (nepravda) nebo 1 (pravda). V tomto se však nachází jisté omezení. U předešlého příkladu jsme, kromě jiných, uvažovali výrok "$x$ je obyvatelem Prahy.", který jsme značili výrokovou proměnnou $P(x)$. Tím jsme přiřadili $P(x)$ jistý význam.\par
